...
截至 2026-09-12，项目处于 **SmartPark 0.6**，SmartPark 0.7 预约系统已完成需求设计、尚未开始编码：
...
- 预约现状仅为 `ParkingService::reserve(ttl)` 短时锁位；远程时间段预约、预约订单表、定金交易、爽约处理和用户端路线接口均待实现。
...
1. **P0 — 预约领域核心**：实现预约模型、未来 7 天时间校验、时间段冲突、定金模拟支付、取消/到场/爽约状态机、SQLite 事务与单元测试。
...
7. ⬜ 预约核心：实现未来 7 天远程预约、定金预付、取消/到场/爽约结算、预期路线和 SQLite 持久化。
8. ⬜ 服务端：以 TCP 建立管理员端、用户端与服务端架构，开放预约和停车事件接口。
9. ⬜ 用户端与出入口终端：用户端提交预约并查看路线；Gate 手动输入车牌并通过服务端核销预约。
...
- 下一步从领域模型、SQLite 表和核心单元测试开始实现，再接 TCP 与用户端。
